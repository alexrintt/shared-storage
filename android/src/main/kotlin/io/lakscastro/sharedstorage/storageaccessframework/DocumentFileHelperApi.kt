...
import io.lakscastro.sharedstorage.storageaccessframework.lib.EXCEPTION_ACTIVITY_NOT_FOUND
import io.lakscastro.sharedstorage.storageaccessframework.lib.EXCEPTION_CANT_OPEN_DOCUMENT_FILE
import io.lakscastro.sharedstorage.storageaccessframework.lib.OPEN_DOCUMENT_FILE
import io.lakscastro.sharedstorage.storageaccessframework.lib.PLUGIN_FILE_PROVIDER_PACKAGE_NAME
...
internal class DocumentFileHelperApi(private val plugin: SharedStoragePlugin) :
  ...
  private fun openDocumentFile(call: MethodCall, result: MethodChannel.Result) {
    ...
    val intent =
      Intent(Intent.ACTION_VIEW).apply {
        ...
        val uriWithProviderScheme = Uri.Builder().let {
          ...
          it.query(uri.query)
          it.authority(
            "${plugin.context.applicationContext.packageName}.$PLUGIN_FILE_PROVIDER_PACKAGE_NAME"
          )
          it.build()
        }
    ...
    try {
      startActivity(plugin.context, intent, null)
