...
import 'package:shared_storage_platform_interface/src/utilities.dart';
...
class MethodChannelNsdPlatform extends NsdPlatformInterface {
  final _methodChannel = const MethodChannel('io.alexrintt/shared_storage');
  ...
  Future<Discovery> startDiscovery(String serviceType,
    ...
    if (isIpLookupEnabled(ipLookupType) && autoResolve == false) {
      throw SharedStorageError(ErrorCause.illegalArgument,
  ...
  Future<dynamic> handleMethodCall(MethodCall methodCall) async {
    ...
    if (handle == null) {
      throw SharedStorageError(ErrorCause.illegalArgument, 'Expected handle');
    }

    final handler = _getHandler(handle, method);
    if (handler == null) {
      throw SharedStorageError(
          ErrorCause.internalError, 'No handler: $method $handle');
    }
...
SharedStorageError toNsdError(Exception e) {
  if (e is! PlatformException) {
    return SharedStorageError(ErrorCause.internalError, e.toString());
  }
  ...
  if (errorCode == null) {
    return SharedStorageError(ErrorCause.internalError, message);
  }

  return SharedStorageError(errorCode, message);
}
